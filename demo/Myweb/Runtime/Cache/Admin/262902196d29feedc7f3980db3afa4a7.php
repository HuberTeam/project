...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius del"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="/demo/index.php/admin/Member/add_group_show?cid=<?php echo ($cid['cid']); ?>" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加公司分组成员</a>   </div>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="100">群组成员名字</th>
				<th width="100">群组成员成员职位</th>
				<th width="100">群组成员成员签名</th>
				<th width="100">群组成员成员头像</th>
				<th width="100">群组成员联系号码</th>
				<th width="100">群组成员性别</th>
				<th width="100">群组成员年龄</th>
				<th width="100">编辑</th>
			</tr>
		</thead>
		<tbody>
		<?php if($result != ''): if(is_array($result)): foreach($result as $key=>$row): ?><tr class="text-c">
				...
				<td><img src="/demo/Uploads/<?php echo ($row["image"]); ?>" width="100px" height="100px" alt=""></td>
				<td><?php echo ($row["phone"]); ?></td>
				<td><?php echo ($row["sex"]); ?></td>
				<td><?php echo ($row["age"]); ?></td>
				<td class="td-manage">  <a title="删除" href="javascript:;" onclick="member_del(<?php echo ($row['id']); ?>,<?php echo ($gid); ?>)" id="del" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
<script type="text/javascript">
	function member_del(id,gid){
		// 要删除的成员ID
		var mid = id;
		var gid = gid;
		$.ajax({
            type: 'POST',
            url: '/demo/index.php/admin/Member/del_group_member',
            data: {mid:id,gid:gid},
